...
int main() {
  ...
  LoadData(GetDataDir(), x_train, t_train, x_test, t_test);
  ...
  ozcode::TwoLayerNet network(input_size, hidden_size, output_size);
  ...
  ozcode::Optimizer* optimizer = new ozcode::OptimizerSgd(learning_rate);
  ...
  for (arma::uword i = 0; i != iter_num; i++) {
    ...
    if (i % iter_per_epoch == 0) {
      double train_acc = network.GetAccuracy(x_train, t_train);
      double test_acc = network.GetAccuracy(x_test, t_test);
      vec_train_acc.push_back(train_acc);
      vec_test_acc.push_back(test_acc);
      std::cout << "train acc, test_acc: " << train_acc << ", " << test_acc
                << std::endl;
      out_file << "train acc, test_acc: " << train_acc << ", " << test_acc
               << std::endl;
    }
